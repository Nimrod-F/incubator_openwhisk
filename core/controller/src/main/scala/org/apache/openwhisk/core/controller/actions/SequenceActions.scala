...
protected[actions] trait SequenceActions {
  ...
  private def invokeProgramComponents(
      ...
      if (accounting.shortcircuit) {
        ...
        if (payloadMap contains "action") {
          ...
          if (!(nameToActions contains next)) {
            ...
            invokeNextAction(user, nextAction, accounting, cause, transid).flatMap(iterateProgramAction(_))
          }
        } else {
    ...
    invokeNextAction(user, resolvedFutureActions(0), accounting, cause, transid)
  ...
  private def invokeNextAction(user: Identity,
    ...
    futureAction.flatMap { action =>
      ...
      val maybeJsObject: Option[JsObject] = inputPayload.flatMap {
      ...
      val futureWhiskActivationTuple = action.toExecutableWhiskAction match {
        case None if action.exec.isInstanceOf[ProjectionExecMetaData] =>
          ...
          invokeAction(user, action, maybeJsObject, waitForResponse = Some(timeout), cause) map {
