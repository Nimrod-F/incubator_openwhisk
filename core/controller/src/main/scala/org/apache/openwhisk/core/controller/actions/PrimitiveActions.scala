...
import org.apache.openwhisk.core.controller.{ProjectionDSL, WhiskServices}
...
protected[actions] trait PrimitiveActions {
  ...
  protected[actions] def invokeApp(
    ...
    if(payloadMap contains("input")){
      ...
      val inputEntry: Option[JsValue] = payloadMap.get("input")

      // Construct a JsObject with the "input" key if it exists
      newPayload = inputEntry match {
        case Some(value) => JsObject("input" -> value)
        case None => JsObject()
      }
    }
    else {
  ...
  protected[actions] def invokeFork(
    ...
    if ((payloadMap contains "input") && (payloadMap contains "saved")) {
      // newPayload = (payloadMap getOrElse ("input", JsObject.empty))
      val inputEntry: Option[JsValue] = payloadMap.get("input")
              ...
              response.map {
                case Right (activation) => {
                  ...
                  if (errorField.isEmpty) {
                    val ActivationResponse (statusCode, responseJsObject, size) = _response
